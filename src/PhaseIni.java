...
public class PhaseIni {
	...
	public void setBasicVariables(int[] basicVariables) {
	...
	public RealMatrix Create(int n, int m, double[][] A, double[] b,
		...
		return Phase1; // It returns Phase 1 Initial with Z and RHS value just
						// like Assigment Paper
	}
	...
	public void basicVars(double[][] A, RealMatrix Phase0, int m, int n) {
		...
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
				if (realA.getColumnVector(j).equals(
						realIdentity.getColumnVector(i))) {
					basicVariables[i] = j;
				}
			}
	...
	public RealMatrix rowOperations(int basicVar, RealMatrix Phase0, int m,
			int n) {
